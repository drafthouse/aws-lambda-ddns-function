...
#    internal_cname = raw_internal_cname + "i" if raw_internal_cname else None
    ...
    print "Internal host name: " + internal_host_name

    if private_hosted_zone_name in private_hosted_zone_collection:
        print 'Private zone found:', private_hosted_zone_name
...
#                create_resource_record(private_hosted_zone_id, private_host_name, private_hosted_zone_name, 'A', private_ip)
                create_resource_record(private_hosted_zone_id, internal_host_name, private_hosted_zone_name, 'A', private_ip)
#                create_resource_record(private_hosted_zone_id, internal_host_name, private_hosted_zone_name, 'CNAME', private_dns_name)
#                create_resource_record(reverse_lookup_zone_id, reversed_ip_address, 'in-addr.arpa', 'PTR', private_dns_name)
                create_resource_record(reverse_lookup_zone_id, reversed_ip_address, 'in-addr.arpa', 'PTR', internal_host_name + "." + private_hosted_zone_name)
                if internal_cname:
#                    create_resource_record(private_hosted_zone_id, internal_cname, private_hosted_zone_name, 'CNAME', private_dns_name)
                    create_resource_record(private_hosted_zone_id, internal_cname, private_hosted_zone_name, 'CNAME', internal_host_name + "." + private_hosted_zone_name)
            except BaseException as e:
                print e
        else:
            try:
                delete_resource_record(private_hosted_zone_id, private_host_name, private_hosted_zone_name, 'A', private_ip)
                delete_resource_record(private_hosted_zone_id, internal_host_name, private_hosted_zone_name, 'A', private_ip)
                delete_resource_record(private_hosted_zone_id, internal_host_name, private_hosted_zone_name, 'CNAME', private_dns_name)
#                delete_resource_record(reverse_lookup_zone_id, reversed_ip_address, 'in-addr.arpa', 'PTR', private_dns_name)
                delete_resource_record(reverse_lookup_zone_id, reversed_ip_address, 'in-addr.arpa', 'PTR', internal_host_name + "." + private_hosted_zone_name)
                if internal_cname:
#                    delete_resource_record(private_hosted_zone_id, internal_cname, private_hosted_zone_name, 'CNAME', private_dns_name)
                    delete_resource_record(private_hosted_zone_id, internal_cname, private_hosted_zone_name, 'CNAME', internal_host_name + "." + private_hosted_zone_name)
            except BaseException as e:
                print e
        # create PTR record
    else:
        print 'No private zone found for %s' % private_hosted_zone_name
...
#                    create_resource_record(public_hosted_zone_id, public_host_name, public_hosted_zone_name, 'A', public_ip)
                if external_host_name:
                    create_resource_record(public_hosted_zone_id, external_host_name, public_hosted_zone_name, 'CNAME', public_dns_name)
                if external_cname:
#                    create_resource_record(public_hosted_zone_id, external_cname, public_hosted_zone_name, 'CNAME', public_dns_name)
                    create_resource_record(public_hosted_zone_id, external_cname, public_hosted_zone_name, 'CNAME', external_host_name + "." + public_hosted_zone_name)
            except BaseException as e:
                print e
        else:
            try:
                ...
                if external_host_name:
                    delete_resource_record(public_hosted_zone_id, external_host_name, public_hosted_zone_name, 'A', public_ip)
                    delete_resource_record(public_hosted_zone_id, external_host_name, public_hosted_zone_name, 'CNAME', public_dns_name)
                if external_cname:
#                    delete_resource_record(public_hosted_zone_id, external_cname, public_hosted_zone_name, 'CNAME', public_dns_name)
                    delete_resource_record(public_hosted_zone_id, external_cname, public_hosted_zone_name, 'CNAME', external_host_name + "." + public_hosted_zone_name)
            except BaseException as e:
                print e
    else:
        print 'No public zone found for %s' % public_hosted_zone_name
...
def get_dhcp_configurations(dhcp_options_id):
    ...
    print("DHCP Options ID: " + dhcp_options_id)
